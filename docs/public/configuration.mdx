...
| `CLAUDE_MEM_WORKER_HOST`      | `localhost`                     | Worker service host address           |
...
| `CLAUDE_MEM_OPENROUTER_APP_NAME`             | `codex-mem`                | Optional: App name for analytics      |
...
| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `CLAUDE_MEM_DATA_DIR`         | `~/.codex-mem`                 | Data directory location               |
...
```bash
./codex-mem-settings.sh
```

This script manages settings in `~/.codex-mem/settings.json`.
...
- **Production (installed plugin)**: `~/.codex-mem/` (always, regardless of CLAUDE_PLUGIN_ROOT)
- **Development**: Can be overridden with `CLAUDE_MEM_DATA_DIR`

```
~/.codex-mem/
├── codex-mem.db           # SQLite database
...
Settings are stored in `~/.codex-mem/settings.json`:
...
Settings can be customized in `~/.codex-mem/settings.json`.
...
Control which tools are excluded from observations. Edit `~/.codex-mem/settings.json`:
```json
{
