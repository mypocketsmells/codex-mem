...
| `CLAUDE_MEM_OPENROUTER_APP_NAME` | string | `codex-mem` | Optional: App name for analytics |
...
If OpenRouter encounters errors, codex-mem automatically falls back to the Claude Agent SDK:
...
Either:
- Set `CLAUDE_MEM_OPENROUTER_API_KEY` in `~/.codex-mem/settings.json`, or
...
HTTP-Referer: https://github.com/thedotmack/codex-mem
X-Title: codex-mem
