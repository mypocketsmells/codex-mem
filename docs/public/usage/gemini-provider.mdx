...
If Gemini is selected but encounters errors, codex-mem automatically falls back to the Claude Agent SDK:
...
Either:
- Set `CLAUDE_MEM_GEMINI_API_KEY` in `~/.codex-mem/settings.json`, or
...
<Tip>
**Recommended**: Enable billing on your Google Cloud project to unlock much higher rate limits. You won't be charged unless you exceed the generous free quota. This allows codex-mem to process observations instantly instead of waiting between requests.
</Tip>
