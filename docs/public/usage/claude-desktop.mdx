...
description: Use codex-mem memory search in Claude Desktop with MCP tools
icon: desktop
---
...
Claude Desktop can access your codex-mem memory database through **MCP tools**. This allows you to search past sessions, decisions, and observations directly from Claude Desktop conversations.

## Prerequisites

Before configuring MCP tools, ensure:

1. **codex-mem is installed** and the worker service is running
...
1. Ensure codex-mem has recorded sessions (check http://localhost:37777)
2. Verify the database exists: `ls ~/.codex-mem/codex-mem.db`
