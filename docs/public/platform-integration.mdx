...
description: Complete reference for integrating codex-mem worker service into VSCode extensions, IDE plugins, and CLI tools
icon: plug
---

<Note>
**Version:** 7.0.0 (December 2025)
**Target Audience:** Developers building codex-mem integrations (VSCode extensions, IDE plugins, CLI tools)
</Note>
...
**Tag Stripping:** Removes `<private>` and `<codex-mem-context>` tags before storage.
...
```json Response
{
  "projects": ["codex-mem", "other-project", ...]
}
```
...
// Register tool
vscode.lm.registerTool('codex-mem-search', searchTool);
```

#### Chat Participant Implementation

```typescript
const participant = vscode.chat.createChatParticipant('codex-mem', async (request, context, stream, token) => {
...
- `<codex-mem-context>content</codex-mem-context>` - System-level tag (prevents recursive storage)
...
export function registerSearchTool(context: vscode.ExtensionContext) {
  ...
  const searchTool = vscode.lm.registerTool('codex-mem-search', {
    description: 'Search persistent memory for observations, sessions, and prompts',
    inputSchema: {
      type: 'object',
      properties: {
...
```typescript
const userMessage = '<private>API key: <YOUR_API_KEY></private>';
...
<CardGroup cols={2}>
  <Card title="Documentation" icon="book" href="https://codex-mem.ai">
    Complete codex-mem documentation
  </Card>
  <Card title="GitHub" icon="github" href="https://github.com/thedotmack/codex-mem">
