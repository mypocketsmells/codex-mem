...
1. Check browser localStorage is enabled:
   ```javascript
   // In browser console
   localStorage.getItem('codex-mem-settings')
   ```
...
3. Clear localStorage and try again:
   ```javascript
   // In browser console
   localStorage.removeItem('codex-mem-settings')
   ```
...
3. Check database for corruption:
   ```bash
   sqlite3 ~/.codex-mem/codex-mem.db "PRAGMA integrity_check;"
   ```
...
3. Verify database has observations:
   ```bash
   sqlite3 ~/.codex-mem/codex-mem.db "SELECT COUNT(*) FROM observations;"
   ```
...
1. Check if summaries exist:
   ```bash
   sqlite3 ~/.codex-mem/codex-mem.db "SELECT COUNT(*) FROM session_summaries;"
   ```
...
3. Check database size (large database = slow queries):
   ```bash
   ls -lh ~/.codex-mem/codex-mem.db
   ```

4. Optimize database:
   ```bash
   sqlite3 ~/.codex-mem/codex-mem.db "VACUUM;"
   ```
...
2. Backup database:
   ```bash
   cp ~/.codex-mem/codex-mem.db ~/.codex-mem/codex-mem.db.backup
   ```
...
1. Check FTS5 tables exist:
   ```bash
   sqlite3 ~/.codex-mem/codex-mem.db "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts';"
   ```
...
3. Check triggers exist:
   ```bash
   sqlite3 ~/.codex-mem/codex-mem.db "SELECT name FROM sqlite_master WHERE type='trigger';"
   ```
...
2. Verify FTS5 tables populated:
   ```bash
   sqlite3 ~/.codex-mem/codex-mem.db "SELECT COUNT(*) FROM observations_fts;"
   ```
...
**Symptoms**: `/plugin install codex-mem` fails.
...
**Solution**: Check `~/.codex-mem/` exists and is writable.
...
2. **Create issue**: [GitHub Issues](https://github.com/thedotmack/codex-mem/issues)
