...
This document describes the PM2 to Bun migration that occurred in v7.1.0 (December 2025). If you're installing codex-mem for the first time, this migration has already been completed and you can use the current Bun-based system documented in the main guides.
...
- **Logs**: `~/.pm2/logs/codex-mem-worker-*.log`
- **PID File**: `~/.pm2/pids/codex-mem-worker.pid`

**Lifecycle Commands**:
```bash
pm2 start <script>           # Start worker
pm2 stop codex-mem-worker   # Stop worker
pm2 restart codex-mem-worker # Restart worker
pm2 delete codex-mem-worker  # Remove from PM2
pm2 logs codex-mem-worker    # View logs
```
...
- **Port File**: `~/.codex-mem/.worker.port` (new)
...
- Database file: `~/.codex-mem/codex-mem.db` (same location)
...
if (!existsSync(pm2MigratedMarker)) {
  try {
    spawnSync('pm2', ['delete', 'codex-mem-worker'], { stdio: 'ignore' });
...
`ensureWorkerRunning()` checks if `~/.codex-mem/.worker.pid` exists (it doesn't for first run after update)
</Step>
<Step title="Start Worker Decision">
Worker not running → Call `startWorker()`
</Step>
<Step title="Migration Check">
Check if `~/.codex-mem/.pm2-migrated` exists
</Step>
<Step title="PM2 Cleanup">
Execute `pm2 delete codex-mem-worker` (errors ignored), create marker file
</Step>
...
**Location**: `~/.codex-mem/.pm2-migrated`
...
4. **PM2 cleanup**: `pm2 delete codex-mem-worker` (old worker terminated)
5. **Marker creation**: `~/.codex-mem/.pm2-migrated` with timestamp
...
| `pm2 stop codex-mem-worker` | `npm run worker:stop` | Stop worker |
| `pm2 restart codex-mem-worker` | `npm run worker:restart` | Restart worker |
| `pm2 delete codex-mem-worker` | `npm run worker:stop` | Remove worker |
| `pm2 logs codex-mem-worker` | `npm run worker:logs` | View logs |
| `pm2 describe codex-mem-worker` | `npm run worker:status` | Detailed status |
...
**Logs**:
```
Old: ~/.pm2/logs/codex-mem-worker-out.log
     ~/.pm2/logs/codex-mem-worker-error.log

New: ~/.codex-mem/logs/worker-YYYY-MM-DD.log
```

**PID Files**:
```
Old: ~/.pm2/pids/codex-mem-worker.pid

New: ~/.codex-mem/.worker.pid
```
...
New: ~/.codex-mem/.worker.pid
     ~/.codex-mem/.worker.port
     ~/.codex-mem/.pm2-migrated (all platforms)
```

**Database** (unchanged):
```
Same: ~/.codex-mem/codex-mem.db
```
...
├────┼────────────────────┼─────────┼─────────┼──────────┤
│ 0  │ codex-mem-worker  │ online  │ 0       │ 2d 5h    │
└────┴────────────────────┴─────────┴─────────┴──────────┘
```
...
# Or just remove codex-mem logs
rm -f ~/.pm2/logs/codex-mem-worker-*.log
rm -f ~/.pm2/pids/codex-mem-worker.pid
```
...
```
~/.codex-mem/
├── codex-mem.db          # Database (unchanged)
...
~/.pm2/
├── logs/
│   ├── codex-mem-worker-out.log
│   └── codex-mem-worker-error.log
├── pids/
...
```
~/.codex-mem/
├── codex-mem.db          # Database (same file)
├── chroma/                # Vector embeddings (unchanged)
├── logs/
...
**Symptoms**:
- `pm2 list` still shows `codex-mem-worker`
...
**Manual Resolution**:
```bash
rm ~/.codex-mem/.worker.pid
rm ~/.codex-mem/.worker.port
...
# 2. Start PM2 worker
pm2 start plugin/scripts/worker-cli.js --name codex-mem-worker
...
# 5. Verify migration
pm2 list  # Should NOT show codex-mem-worker
cat ~/.codex-mem/.pm2-migrated  # Should exist
